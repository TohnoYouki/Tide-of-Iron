...
public class PolyNavAgent : MonoBehaviour{
	...
	void OnEnable(){ allAgents.Add(this); old_goal = gameObject.transform.position; }
    ...
    private Vector2 old_goal;
    public bool MySetDestination(Vector2 goal)
    {
        if (goal != old_goal)
        {
            openstart = true; open_min_mode = true; time = 0.0f;
        }
        old_goal = goal;
    ...
    private bool open_min_mode = true;
	void LookAhead(){
		...
		if (avoidRadius > 0){
			...
			for (int i = 0; i < allAgents.Count; i++){
                ...
                if ((!otherAgent.open_min_mode) && (distance < mlt * 2)) 
                    x = false;
            ...
            if (openstart)
                open_min_mode = true;
            else if (open_min_mode && !x)
                open_min_mode = false;
            if ((!open_min_mode) && (velocity.magnitude < min))
                Stop();
            if ((open_min_mode) && (velocity.magnitude < minSpeed))
